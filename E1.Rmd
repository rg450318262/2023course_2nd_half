...
For reading convenience, I collapsed the chunks. Please unfold the chunk to check the codes, whenever needed.
...
This summary, after removing the three columns, is `r summary`.
...
In this chunk, I made a function to do k fold validation using MSE as loss function:
...
In this chunk, I use the function created above and generated the table as required by the question.
...
A function has been made in the chunk to do k fold validation:
...
In the chunk, I read in data `test_real.csv`:
...
In the chunk, I subset data into train and test
...
This chunk assigns the predefined values to objects:
...
In this chunk, I run a loop that train polynomial functions p_hat(x) with degree of p, through 1000 simulated data points:
...
In the chunk, the plots are generated. 
...
Prove that $$\mathbb{E}[L_{\text{train}}] ≤ \mathbb{E}[L_{\text{test}}]$$ .
...
I read in the data d1, d2, d3 and d4:
...
In the chunk below, I fit 4 models for d1 to d4 data sets:
...
In the chunk below, I generate a table showing the intercept and slope estimates for the models, and relevant statistics. Note that numbers 1 to 4 in column `model_NO` correspond to models fitted for 4 data sets.
...
This function created in the chunk removes an x from a dataset each time,  refit the model, and calculate the MSE change from initial MSE (all x reserved model). Outliers that have large influence on the loss will be detected. 
...
test.outlier <- function(data, y, x){
  ...
  outlier.table <- data.frame(
    "Description" = NA, "x removed"= NA, "y removed"= NA,"MSE"= NA, "MSE change" = NA
    )
  #create id variable
  data$id <- 1:nrow(data)
...
From the table above, I found data 3 suffers from an outlier. Removal of x in row 3 resulted in 99.99% MSE change.

Besides, the leverage vs studentized residual plot below also shows there is an out-lier with medium level of leverage data point in data 3 model.
...
This chunk a function is made to calculate the standard error for regression coefficient, using bootstrapping technique.
...
The standard errors for regression coeﬀicient estimates for the data set d2 is `r se`, generated from codes above
...
In bootstrap, you sample 𝑛 data points from a population of 𝑛 points with replacement. Argue that the probability that the 𝑗th observation is not in the bootstrap sample is about 0.368 when 𝑛 is very large.
