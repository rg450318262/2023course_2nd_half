...
knitr::opts_chunk$set(collapse = TRUE, warning = T, message = T, echo = F)
```
...
baseline_test_accuracy<- get_accuracy(fit, test.set, "SPAM")[1,2]|> 
  kable(format = "latex",
        ...
        escape = TRUE, 
        digits = 5,
        caption = "accuracy of training and testsing set"
        ) 
```
...
where $k$ is the class index (1 or 2), $\mu_k$ is the mean of $k$, and $\sigma_k^2$ is the variance of class $k$.
...
where $ a $, $ b $, and $ c $ are coefficients that depend on $\mu_1 $, $\mu_2$, $\sigma_1^2 $, and $ \sigma_2^2 $.

If $\sigma_1^2 = \sigma_2^2$, the $x^2$ terms cancel out, and the decision boundary becomes linear. However, if $\sigma_1^2 \neq \sigma_2^2$, the $x^2$ term remains, making the decision boundary quadratic, hence non-linear.
...
p(x) = \left( \prod*{j=1}^{4}*\frac{1}{\sqrt{2\pi\sigma_{\text{Adelie},j}^2}}e^{-\frac{(x_j -\mu_{\text{Adelie},j})^2}{2\sigma_{\text{Adelie},j}^2}}\right) p(\text{Adelie}) + \left( \prod{j=1}^{4}\frac{1}{\sqrt{2\pi\sigma_{\text{notAdelie},j}^2}}e^{-\frac{(x_j - \mu_{\text{notAdelie},j})^2}{2\sigma_{\text{notAdelie},j}^2}}\right) p(\text{notAdelie})
...
c.  In the study, the authors experimented on the performance of naive
...
d.  Also note that generative models are often more naturally equipped
    to handle missing data than discriminative models. The comparison
    above is built upon the scenario when no missing values exist in the
    data.
...
For 1-NN classifier, the classification boundaries are **4, 5.5, 10.5,
15.5 and 17**. The classification error is **0**.

For 3-NN classifier, the classification boundaries are **5.5, 10.5,
17**. The classification error is **0.143**.
